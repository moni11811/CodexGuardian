...
Hard restart requests remain on disk while Guardian reads bounded rollout headers/tails. It scans every rollout changed since the current Codex process launched, with at least a 12-hour lookback. Every non-terminal task blocks restart except the active recovery-heartbeat turn carrying the request's exact origin UUID. Newly resumed work in that same task blocks again. Missing task state, more than 200 candidates, malformed scans, or scanner failure also blocks restart. Corrupt request files are preserved separately and do not poison valid requests. The native heartbeat must call `recovery_tick` once before Guardian permits the restart. This proves Codex registered it. Guardian verifies it again immediately before stopping Codex. Once unrelated tasks are terminal and no rollout changes for 15 seconds, Guardian takes a bounded state lock, claims the exact queue snapshot, checks activity again, and restarts Codex once. It waits for a new Codex app-server process, then gives that process a full 15-second startup window before opening `codex://threads/<TASK_ID>` and releasing continuation. One heartbeat run receives the on-device recovery prompt; overlapping runs wait. The claim remains until the recovered task deletes the heartbeat and calls `ack_recovery`. Guardian never launches detached `codex exec resume` workers. If the origin, heartbeat, activity, lock, or startup cannot be proven, Codex is not restarted.
